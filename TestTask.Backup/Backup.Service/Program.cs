using Backup.Core.Interfaces;
using Backup.Core.Model;
using System.Linq;
using System.ServiceProcess;

namespace Backup.Service
{
    static class Program
    {
        /// <summary>
        /// The main entry point for the application.
        /// </summary>
        static void Main(string[] args)
        {
            if (!args.Any()) return;
            bool isManual = args.Any() && args.First().ToLower() == "/console";
            string configurationPath = args[isManual ? 1 : 0];
            Configuration configuration = Configuration.CreateConfiguration(configurationPath);

            IBackupServiceFactory factory = ServiceFactory.CreateFactory(configuration);
            ServiceBase backupService = new BackupService(factory);

            if (isManual)
            {
                ((BackupService)backupService).ManualRun();
                return;
            }

            ServiceBase[] ServicesToRun;
            ServicesToRun = new ServiceBase[]
            {
                backupService
            };
            ServiceBase.Run(ServicesToRun);
        }
    }
}
